...
*Setup instructions for a Gitian build of Modestcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Modestcoin
...
- [Building Modestcoin Core](#building-modestcoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-modestcoin-host/README.md).
...
Clone the git repositories for modestcoin and Gitian.
...
in the modestcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Modestcoin Core
----------------

To build Modestcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the modestcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/modestcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/modestcoin-project/modestcoin
...
For example:
```bash
URL=https://github.com/thrasher-/modestcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit modestcoin=${COMMIT} --url modestcoin=${URL} ../modestcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit modestcoin=${COMMIT} --url modestcoin=${URL} ../modestcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit modestcoin=${COMMIT} --url modestcoin=${URL} ../modestcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the modestcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../modestcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/modestcoin
SIGPATH=/some/root/path/modestcoin-detached-sigs

./bin/gbuild --url modestcoin=${BTCPATH},signature=${SIGPATH} ../modestcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/modestcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/modestcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/modestcoin-osx-build.assert
```
...
[modestcoin-project/gitian.sigs.ltc](https://github.com/modestcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
