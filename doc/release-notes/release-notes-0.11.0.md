Modestcoin Core version 0.11.0 is now available from:

  <https://modestcoin.org/bin/modestcoin-core-0.11.0/>
...
For example, add the following to `modestcoin.conf`:
...
related to the blockchain in the modestcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Modestcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Modestcoin Core will 
...
For an example script see [modestcoin-submittx](https://github.com/laanwj/modestcoin-submittx).
...
- #5334 `cf87536` libmodestcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new modestcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only ModestcoinAmountField
- #5753 `0f44672` Add modestcoin logo to about screen
...
- #5809 `46bfbe7` Add modestcoin-cli man page
...
- #5849 `41259ca` contrib/init/modestcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Modestcoin-Qt.app to Modestcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for modestcoin-tx)
