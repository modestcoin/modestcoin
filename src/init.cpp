 ...
 */
static const char* BITCOIN_PID_FILENAME = "modestcoind.pid";
...
//        LogPrintf("Config file: %s (not found, skipping)\n", config_file_path.string());
        ...
        if (configFile != NULL) {
            std::string strHeader = "# MODESTCOIN(MTC) config file\n"
                          "rpcuser=username\n"
...
//        return; // Nothing to read, so just return
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if modestcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if modestcoin is started while in a temporary directory.\n",
