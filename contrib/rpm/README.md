...
The RPM spec file provided here is for Modestcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://modestcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Modestcoin git repository](https://github.com/modestcoin/modestcoin) but are not part of
...
Currently only `Source10` is used, for the example `modestcoin.conf` file.
...
in the modestcoin git repository. At present only an SVG version of the Modestcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the modestcoin
...
needed when 0.12.1 is released, a proper fix is already in the Modestcoin git
master branch.
...
idea to build Modestcoin Core against a static version of that library compiled
...
The `modestcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `modestcoin-server` package. At this point, most RPM based
...
The `modestcoin-server` package should have SELinux support. How to properly do
...
Modestcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/modestcoin/). He has been
packaging Modestcoin for Fedora at least since 2012.
...
4. I renamed the `modestcoin` package that contains the Qt GUI to `modestcoin-core` as
that appears to be how the general population refers to it, in contrast to
`modestcoin-xt` or `modestcoin-classic`. I wanted to make sure the general population
